...
public interface ReviewRepository  extends CrudRepository<Review, Integer> {
	...
	@Query(value = "SELECT r.id, r.comment, r.rating , r.tbl_product_id as rproduct, r.created_at, r.tbl_user_id as ruser, u.email as remail FROM reviews r inner join tbl_product p on p.id = r.tbl_product_id inner join tbl_user u on u.id = r.tbl_user_id where r.tbl_product_id =?1", nativeQuery = true)
